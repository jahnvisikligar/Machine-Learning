...
Sample_1 = np.array([[1, 1650, 3], [1, 3000, 4]])


N_1 = Sample_1.shape[0]

mean_vector_1 = np.mean(Sample_1, axis=0)
std_vector_1 = np.std(Sample_1, axis=0)

mean_vector_1 = np.expand_dims(mean_vector_1, axis=0)
std_vector_1 = np.expand_dims(std_vector_1, axis=0)

repeated_mean_1 = np.repeat(mean_vector_1, N_1, axis=0)
repeated_std_1 = np.repeat(std_vector_1, N_1, axis=0)

if np.any(repeated_std_1 == 0):
    repeated_std_1 += np.finfo(float).eps

X_normalized_1 = (Sample_1 - repeated_mean_1) / repeated_std_1

for i in range(0, N_1):
    hypothesis_1 = calculate_hypothesis(X_normalized_1, theta_final, i)
    print(i, " : ", hypothesis_1)
